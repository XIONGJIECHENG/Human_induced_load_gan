...
weight_path = 'F:\\GAN_human_load\\check_point\\'    #the path you store trained weights
gene_sample_path = 'F:\\GAN_human_load\\gene_sample\\'  #the path you generate sample
number_gene_sample = 1000  #number of generated samples for each type of load
...
saver.restore(sess, weight_path+'my_model-1000000')

y_sample = np.zeros(shape=[number_gene_sample, y_dim])

for i in range(12):
    y_sample[:,i]=1
    samples = sess.run(G_sample, feed_dict={z: sample_z(number_gene_sample, z_dim), y:y_sample})
    y_sample = np.zeros(shape=[number_gene_sample, y_dim])
    sio.savemat(gene_sample_path+'gene_samples{}.mat'.format(str(i+1)), {'Gene_load': samples}) 